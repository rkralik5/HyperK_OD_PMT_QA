...
Mainly discuss safety considerations (any heavy lifting work? anything else?), description of the work - what part of the work will be done by whom exactly (shifter 1, shifter 2).

What kind of team organisation is there going to be? (shift leader and one in training), how does it align with the OD PMT assembly?

If we would like to hire external workers (like Toyama university students)


What is the general purpose of the Quality Assurance (QA) process for the PMTs.

Describe the space where we will do the QA - maybe only reference Sammy's technote?
...
For construction, we need to deliver 120 PMTs per day of OD construction
...
How much do we need to mention here? Can we just reference Sammy's technote?
...
\subsection{Overview}
\begin{enumerate}
    \item Finish measurements from the previous day/week (expert)
    \begin{enumerate}
    ...
    \end{enumerate}
    \item Unload PMTs from the dark boxes (shifter)
    \begin{enumerate}
    ...
    \end{enumerate}
    \item Visual inspection of the PMTs (shifter)
    \begin{enumerate}
      \item If not already running, start the QA program
      \item One by one perform the visual inspection of the PMTs
    \end{enumerate}
    \item Load the PMTs into the dark boxes (shifter)
    \begin{enumerate}
      \item Load the PMTs one by one - note the position that the PMT is being loaded to
      \item scan the bar code and connect the PMT to the correct corresponding splitter boards
