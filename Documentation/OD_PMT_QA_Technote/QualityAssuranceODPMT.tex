...
We require that there are no flasher PMTs. We define flashers as PMTs that produce a signal of 10 photo-electrons or more in a neighbouring PMT, while both are at operational voltage. If the recorded signal is lower than 10 photo-electrons, they can be ignored as we are able to filtered this effect out in later reconstruction.

We require that the PMT response for different charge inputs is linear within $<10\%$, up to a 50 photo-electron light input. Only 0.2\% of hits in the OD are above 50 photo-electrons. We also require that the PMT do not experience charge saturation under $<50$ photo-electron signal.
\todo[inline]{Is this really what the requirement on charge saturation means?}

\subsection{Single photo-electron characteristics}
The ratio of the single photo-electron charge distribution's peak hight, and of the valley between the single photo-electron peak and the pedestal, should be $>1.8$ for all PMTs, and its average over all the PMTs should be $>2.0$. The charge resolution of the single photo-electron signal should be $<50\%$.
...
We require that the rise time of the PMT is less than 10 ns at gain of $5 \times 10^6$, which is the same as the inner detector PMTs. Hamamatsu claims a raise time of about 5 ns. We also require that a 20 photoelectron signal had $95\%$ of its total charge accumulated within $100\,\text{ns}$.

The overshoot should be within $<0.5\,\text{mV}$ outside of $200\,\text{ns}$ in order to suppress fake triggers. The afterpulse for a 5 photo-electron light should be $<30\%$ within a $10\,\mu\text{s}$ window and at $-3.0\,\text{mV}$ threshold.
...
We require that the variation of a single photo-electron detection effieciency across the photocathode surface is within $\pm10\%$. This has to be true for light source with wavelength between $350 -- 420\,\text{nm}$, with a $5\,\text{mm}$ spot size, and within a $0 -- 32\,\text{mm}$ radius from the centre of the PMT.

Since we will collect significant portion of our light from WLS plates connected at the edges of the PMT, we also require that all PMTs have at most a $\pm15\%$ variation with respect to the centre of the PMT, using a single photo-electron light with wavelength $430--460\,\text{nm}$ and a $5\,\text{mm}$ sport size. The radius of the WLS region is defined as $32 -- 37\,\text{mm}$ from the centre of the PMT.
...
Based on our PMT requirements, we designed the QA to ensure that the PMTs satisfy them and that they will perform up to the required standard within the Hyper-K detector.
